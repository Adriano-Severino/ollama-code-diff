...
function activate(context) {
    ...
    context.subscriptions.push(statusBarButton, showMenuCommand, generateCodeCommand, editCodeCommand, analyzeFileCommand, analyzeProjectCommand, analyzeMultipleFilesCommand, showDiffCommand);
}
// NOVA FUNÇÃO: Menu principal do Ollama
async function showOllamaMenu(ollamaService, diffManager) {
...
async function handleGenerateCode(ollamaService, diffManager) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
...
async function handleEditCode(ollamaService, diffManager) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage('Nenhum editor ativo encontrado');
        return;
...
function buildGeneratePrompt(userPrompt, language, context) {
...
function buildEditPrompt(originalCode, editInstruction, language) {
