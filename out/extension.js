...
function activate(context) {
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(async (document) => {
        // Only index source files and skip node_modules
        const filePath = document.uri.fsPath;
        if (filePath.match(/\.(ts|js|py|java|cs|cpp|c|h|md|json|html|css)$/) && !filePath.includes('node_modules')) {
...
async function handleFixDiagnostic(document, range, diagnostics, ollamaService, diffManager) {
    ...
    try {
        vscode.window.showInformationMessage('🤖 Ollama: Fixing bug...');
        const fixedCode = await ollamaService.generateCode(prompt);
