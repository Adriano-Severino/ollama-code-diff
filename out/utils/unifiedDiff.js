...
exports.applyUnifiedDiffToContent = applyUnifiedDiffToContent;
const HUNK_HEADER_RE = /^@@ -(\d+)(?:,(\d+))? \+(\d+)(?:,(\d+))? @@/;
function sanitizeUnifiedDiff(content) {
    ...
    while (i < lines.length) {
        ...
        while (i < lines.length) {
            ...
            if (consumedOld !== hunk.oldLines || consumedNew !== hunk.newLines) {
                throw new Error(`Hunk incompleto em ${file.newPath || file.oldPath}. Esperado -${hunk.oldLines}/+${hunk.newLines}, ` +
...
function applyUnifiedDiffToContent(originalContent, filePatch) {
    ...
    for (const hunk of filePatch.hunks) {
        ...
        for (const line of hunk.lines) {
            ...
            if (currentLine !== line.content) {
                throw new Error(`Conflito ao aplicar patch em ${filePatch.newPath || filePatch.oldPath}. ` +
...
function isFileHeaderLine(lines, index) {
...
function splitTextForPatch(content) {
...
function joinLinesWithStyle(lines, hasTrailingNewline, eol) {
