...
class RAGService {
    constructor(context, ollamaService) {
        ...
        this.includePattern = '**/*.{ts,tsx,js,jsx,py,java,cs,cpp,c,h,md,json,html,css}';
        this.excludePattern = '**/{node_modules,.git}/**';
        this.gitignoreScanThrottleMs = 2000;
    ...
    loadVectors() {
        if (fs.existsSync(this.dbPath)) {
            try {
                ...
                logger_1.Logger.info(`Carregados ${this.vectors.length} vetores.`);
            }
            catch (e) {
                logger_1.Logger.error('Erro ao carregar vetores.', e);
                this.vectors = [];
            }
        }
    }
            ...
            await vscode.window.withProgress({
                ...
                for (let i = 0; i < files.length; i++) {
                    ...
                    try {
                        ...
                        logger_1.Logger.error(`Erro ao indexar ${fileUri.fsPath}.`, e);
                    }
                ...
                if (!cancelled) {
                    ...
                    this.saveVectors();
                    vscode.window.showInformationMessage(`Indexação concluída! ${updated} atualizados, ${skipped} sem mudanças, ${removed} removidos, ${ignoredByGitignore} ignorados.`);
                    return;
                }
                this.saveVectors();
                vscode.window.showWarningMessage(`Indexação cancelada. ${updated} atualizados, ${skipped} sem mudanças, ${ignoredByGitignore} ignorados.`);
            });
        }
        finally {
            this.isIndexing = false;
    ...
    async indexFile(uri) {
        if (this.isIndexing)
            return;
        if (!(await this.shouldIndexFile(uri)))
            return;
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                if (updated) {
                    this.saveVectors();
                    logger_1.Logger.debug(`RAG: Arquivo ${path.basename(pathStr)} reindexado.`);
                }
            }
            catch (e) {
                logger_1.Logger.error(`Erro ao reindexar arquivo ${pathStr}.`, e);
            }
    ...
    async processFile(fileUri) {
        try {
            ...
            logger_1.Logger.error(`Erro ao processar arquivo ${fileUri.fsPath}.`, e);
            return false;
        }
    }
    async collectIndexableFiles(workspaceFolders, forceRefresh = false) {
        const files = [];
    ...
    async getGitignoreEntries(folder, forceRefresh = false) {
        ...
        const gitignoreUris = await vscode.workspace.findFiles(new vscode.RelativePattern(folder, '**/.gitignore'), this.excludePattern);
        ...
        for (const uri of gitignoreUris) {
            ...
            try {
                ...
                logger_1.Logger.warn(`Erro ao ler .gitignore em ${filePath}.`, e);
            }
        }
    ...
    isIgnoredByGitignore(fileUri, entries) {
        if (entries.length === 0)
            return false;
    ...
    async shouldIndexFile(uri) {
        ...
        if (entries.length > 0 && this.isIgnoredByGitignore(uri, entries))
            return false;
        return true;
    }
    ...
    async search(query, limit = 5) {
        ...
        try {
            ...
            logger_1.Logger.warn('Falha na expansão da query RAG.', e);
        }
